...
use nalgebra::{IsometryMatrix3, Matrix3, RealField, Rotation3, SMatrix, Scalar, Translation3};
    ...
    T: RealField,
{
    pub fn predict_cov(&mut self, dt: T) {
        ...
        fx.sensitivity_mut::<AngularAccState<T>, RotationState<T>>()
            .copy_from(&Matrix3::from_diagonal_element(dt.clone()));
...
where
    T: RealField,
{
    fn predict(&mut self, delta: DeltaTime<T>) {
        self.eskf.predict(delta)
    }
}
